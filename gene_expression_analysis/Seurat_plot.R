...
DrPlot <- function(object, dr = "pca", dr_plot = "pca", pcs = 50, pval = 1e-05, non.rand.sd.frac = 0.5, k, res, in_dir, out_dir, dr_type = "PCA") {
	...
	for (kk in k) {
		for (r in res) {
			...
			pdf(file.path(out_dir, paste0(dr_type, "_plot_", cl_ident, ".pdf")), width = 3.7, height = 3)
	...
	if (feature_method == "mean.var.plot") {
		for (ymin in disps) {
			...
			if (!(tsne_dr %in% names(object@reductions)))
				object <- RunTSNE(object, reduction = dr, reduction.name = tsne_dr, reduction.key = paste0("tSNE_", dr, "_"))
			DrPlot(object, dr = dr, dr_plot = dr, k = k, res = res, in_dir = in_dir, out_dir = out_dir, dr_type = "PCA")	
			DrPlot(object, dr = dr, dr_plot = tsne_dr, k = k, res = res, in_dir = in_dir, out_dir = out_dir, dr_type = "TSNE")			
		}
	} else { 
		for (nfeat in nfeats) {
			...
			dir.create(out_dir, showWarnings = FALSE)
			if (!(tsne_dr %in% names(object@reductions)))
				object <- RunTSNE(object, reduction = dr, reduction.name = tsne_dr, reduction.key = tsne_dr)
			DrPlot(object, dr = dr, dr_plot = dr, k = k, res = res, in_dir = in_dir, out_dir = out_dir, dr_type = "PCA")	
			DrPlot(object, dr = dr, dr_plot = tsne_dr, k = k, res = res, in_dir = in_dir, out_dir = out_dir, dr_type = "TSNE")
		}
	}
