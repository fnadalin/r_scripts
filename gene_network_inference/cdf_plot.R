...
for (i in 1:n) {
	...
	auc_steps <- (0:100-0.5)*max(df$AUC)/100
	cls <- sort(unique(df$cluster))
	plot(x = c(0,max(df$AUC)), y=c(0,1), pch = NA, xlab = "AUC", ylab = "fraction of cells", main = regulons[i])
	for (j in 1:length(cls)) {
		AUC <- df$AUC[df$cluster == cls[j]]
		auc_cdf <- ecdf(AUC)
		lines(auc_steps, 1-auc_cdf(auc_steps), lwd = 1.5, col = palette[j])
	}
	if (plot_legend)
